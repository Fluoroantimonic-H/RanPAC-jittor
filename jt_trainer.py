...
import copy
import logging
from jt_RanPAC import Learner
from jt_toolkit import count_parameters
...
def _train(args):
    ...
    for task in range(num_tasks):
        ...
        if args['do_not_save'] == False:
            classes_df[col1] = np.pad(predicted_classes, (0, data_manager._test_data.shape[0] - len(predicted_classes)),
                                      'constant', constant_values=(-1, -1))
            classes_df[col2] = np.pad(true_classes, (0, data_manager._test_data.shape[0] - len(predicted_classes)),
                                      'constant', constant_values=(-1, -1))
        model.after_task()
...
def _set_random():
    """设置随机种子（Jittor版本）"""
    jt.set_global_seed(1)  # 设置全局随机种子（包含CPU和CUDA）

    # Jittor特有的确定性配置
    # jt.flags.deterministic = 1  # 启用确定性算法
    # jt.flags.benchmark = 0  # 禁用基准优化
    # jt.flags.cudnn_benchmark = False
